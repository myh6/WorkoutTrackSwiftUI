...
final class SwiftDataExerciseStoreTests: XCTestCase {
    ...
    func test_update_hasNoSideEffectsOnEmptyStore() async throws {
        let sut = makeSUT()
        
        try await sut.update(anyExercise())
    ...
    func test_delete_doesNothingToNonExistingExercise() async throws {
        let sut = makeSUT()
        let savedExercise = anyExercise()
        
        try await sut.insert(savedExercise)
        try await sut.delete(anyExercise())
        
        try await expect(sut, toRetrievedWith: [savedExercise], with: .all(sort: .none))
    }
    ...
    //MARK: - Helpers
    typealias ExerciseStore = ExerciseLoader & ExerciseInsertion & ExerciseDeletion & ExerciseUpdate
